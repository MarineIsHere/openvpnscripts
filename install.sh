...
country=FR
org=$(hostname)
read -e -p "Indiquer l'IP venet que vous avez configuré pour cette VM (ex : 10.0.0.3) : " ip
cat > /etc/openvpnip <<EOF
$ip
EOF
...
country=EN
org=$(hostname)
read -e -p "What is the venet IP of this VM (eg: 10.0.0.3) : " ip
cat > /etc/openvpnip <<EOF
$ip
EOF
